...
public class CalculateFullLoanValueHandler : IEventHandler<CalculateFullLoanValueEvent>
{
    ...
    public CalculateFullLoanValueHandler(ICalculationService<CalculateContractValuesEvent, decimal> calculationService,
        ILogger<CalculateFullLoanValueHandler> logger, IConfiguration config, KafkaProducerService producer)
    {
        _logger = logger;
    ...
    public async Task HandleAsync(CalculateFullLoanValueEvent calculationEvent, CancellationToken cancellationToken)
    {
        try
        {
            var mappedEvent = new CalculateContractValuesEvent(new Guid(), calculationEvent.LoanAmount,
                calculationEvent.LoanTermMonths, calculationEvent.InterestRate, calculationEvent.PaymentType,
        ...
        {
            _logger.LogError("Failed to handle CalculateFullLoanValueEvent. OperationId: {OperationId}. Exception: {e}",
                calculationEvent.OperationId, e.Message);
        }
    }
}