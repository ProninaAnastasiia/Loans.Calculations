...
public class FullLoanValueCalculationService : ICalculationService<CalculateContractValuesEvent, decimal>
{
    ...
    public async Task<decimal> CalculateAsync(CalculateContractValuesEvent calculationEvent,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            var psk = calculationEvent.PaymentType.ToLower() switch
            {
    ...
    private async Task<decimal> CalculateAnnuityPskAsync(CalculateContractValuesEvent calculationEvent,
        decimal coefficient, CancellationToken cancellationToken)
    {
    ...
    private async Task<decimal> CalculateDifferentiatedPskAsync(CalculateContractValuesEvent calculationEvent,
        decimal coefficient, CancellationToken cancellationToken)
    {
    ...
    {
        var delayMilliseconds = new Random().Next(100, 500); // Случайная задержка от 0.1 до 0.5 секунд
