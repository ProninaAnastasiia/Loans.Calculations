...
public class LoanCalculationService : ICalculationService<CalculateContractValuesEvent, ContractValuesCalculatedEvent>
{
    ...
    public LoanCalculationService(ICalculationService<CalculateContractValuesEvent, decimal> calculationService,
    ...
    public async Task<ContractValuesCalculatedEvent> CalculateAsync(CalculateContractValuesEvent request,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                case "аннуитет":
                    monthlyPaymentTask = Task.FromResult(CalculateAnnuityMonthlyPayment(request.LoanAmount,
                        request.InterestRate, request.LoanTermMonths));
                    totalPaymentTask = Task.FromResult(CalculateAnnuityTotalPayment(request.LoanAmount,
                        request.InterestRate, request.LoanTermMonths));
                    totalInterestPaidTask = Task.FromResult(CalculateAnnuityTotalInterestPaid(request.LoanAmount,
                        request.InterestRate, request.LoanTermMonths));
                    break;

                case "дифф":
                    monthlyPaymentTask =
                        Task.FromResult(decimal.Round(CalculateMonthlyPayments(request).AveragePayment, 2));
                    totalPaymentTask =
                        Task.FromResult(decimal.Round(CalculateMonthlyPayments(request).TotalInterest, 2));
                    totalInterestPaidTask =
                        Task.FromResult(decimal.Round(CalculateMonthlyPayments(request).TotalPayment, 2));
                    break;
    ...
    private async Task<decimal> CalculateFullLoanValueAsync(CalculateContractValuesEvent request,
        CancellationToken cancellationToken)
    {
    ...
    private decimal CalculateAnnuityMonthlyPayment(decimal loanAmount, decimal interestRate, int loanTermMonths)
    {
        ...
        var annuityCoefficient = monthlyInterestRate *
                                 (decimal)Math.Pow(1 + (double)monthlyInterestRate, loanTermMonths) /
    ...
    private (decimal AveragePayment, decimal TotalInterest, decimal TotalPayment) CalculateMonthlyPayments(
        CalculateContractValuesEvent request)
    {
